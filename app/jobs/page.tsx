...
const JOB_TYPES = ["Alle", "Timebasis", "Deltid", "Fuldtid", "Projektbaseret"] as const;
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <main className="pt-28 pb-24 px-5">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Header */}
          <div className="mb-10">
            <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3">Jobportal</p>
            <h1 className="text-4xl font-black text-gray-900 mb-2">Ledige stillinger</h1>
            <p className="text-gray-500 text-sm">
              {loading ? "Indlæser…" : `${jobs.length} ${jobs.length === 1 ? "jobopslag" : "jobopslag"} — søg og ansøg direkte`}
            </p>
          </div>
          ...
          <div className="mb-8 space-y-4">
            <input
              ...
              placeholder="Søg på jobtitel, beskrivelse eller lokation…"
          ...
          {/* Job list */}
          {loading ? (
            <div className="text-center py-16 text-gray-400 text-sm">Indlæser jobopslag…</div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-16 text-gray-400">
              <p className="text-sm font-medium">Ingen jobs fundet</p>
              {(search || activeType !== "Alle") && (
                <button
                  onClick={() => { setSearch(""); setActiveType("Alle"); }}
            ...
            <div className="space-y-4">
              {filtered.map((job) => (
                <div
                  key={job.id}
                  className="bg-white border border-gray-100 rounded-2xl p-6 hover:border-gray-300 hover:shadow-sm transition-all"
                >
